package model;

public class Student {
	public static int count = 20;
	public static int max_id = 21;
	private int school_id;
	private int professor_id;
	private int id;
	private String first_name;
	private String surname;
	private String login;
	private String password;
	public Student() {
		this(0, 0, "", "", "abcde", "");
	}

	public Student(int school_id, int professor_id, int id, String first_name, String surname, String login, String password) {
		this.id = id;
		this.professor_id = professor_id;
		this.school_id = school_id;
		this.first_name = first_name;
		this.surname = surname;
		try {
			if (login.length() < 3) throw new Exception("Login is too short.");
			this.login = login;
			this.password = password;
			// this.password = MessageDigest.getInstance("MD5").digest(password.getBytes("UTF-8")).toString();
		}catch(Exception e) {
			System.err.println("ERROR: "+ e.getMessage());
		}
	}
	public Student(int school_id, int professor_id, String first_name, String surname, String login, String password) {
		this.id = count++;
		this.professor_id = professor_id;
		this.school_id = school_id;
		this.first_name = first_name;
		this.surname = surname;
		try {
			if (login.length() < 3) throw new Exception("Login is too short.");
			this.login = login;
			this.password = password;
			max_id = this.id;
			// this.password = MessageDigest.getInstance("MD5").digest(password.getBytes("UTF-8")).toString();
		}catch(Exception e) {
			System.err.println("ERROR: "+ e.getMessage());
		}
	}
	public int getProfessor_id() {
		return professor_id;
	}
	public void setProfessor_id(int professor_id) {
		this.professor_id = professor_id;
	}
	public int getSchool_id() {
		return school_id;
	}
	public void setSchool_id(int school_id) {
		this.school_id = school_id;
	}
	public String getFirst_name() {
		return first_name;
	}
	public void setFirst_name(String first_name) {
		this.first_name = first_name;
	}
	public String getSurname() {
		return surname;
	}
	public void setSurname(String surname) {
		this.surname = surname;
	}
	public String getLogin() {
		return login;
	}
	public void setLogin(String login) {
		this.login = login;
	}
	public String getPassword() {
		return password;
	}
	public void setPassword(String password) {
		this.password = password;
	}
	public int getId() {
		return id;
	}
	public static int getMaxId() {
		return ++max_id;
	}
	public Student clone() {
		Student copy = new Student(school_id, professor_id, id, first_name, surname, login, password);
		return copy;
	}
}
